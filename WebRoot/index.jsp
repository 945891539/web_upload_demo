...
<script type="text/javascript"
	src="${pageContext.request.contextPath }/js/jquery/jquery-1.11.0.js"></script>
<script type="text/javascript"
...
<script type="text/javascript">
	$(document).ready(
			function() {
				$("#uploadify").uploadify(
						{
							'uploader' : 'http://192.168.0.100:8080/Upload',
							'swf' : '${pageContext.request.contextPath }/js/uploadify/uploadify.swf',
							'cancelImg' : '${pageContext.request.contextPath }/images/uploadify-cancel.png',
...
// 										+ ' - 取消上传的文件数: '
// 										+ stats.upload_cancelled + ' - 出错的文件数'
// 										+ stats.queue_errors);
							},
							'onUploadComplete' : function(file,
									swfuploadifyQueue) {//队列中的每个文件上传完成时触发一次  
// 								alert('id: ' + file.id + ' - 索引: ' + file.index
// 										+ ' - 文件名: ' + file.name + ' - 文件大小: '
// 										+ file.size + ' - 类型: ' + file.type
// 										+ ' - 创建日期: ' + file.creationdate
...
// 										+ ' - 文件状态: ' + file.filestatus);
							...
							'onUploadError' : function(file, errorCode,
									errorMsg, errorString, swfuploadifyQueue) {//上传文件出错是触发（每个出错文件触发一次）  
// 								alert('id: ' + file.id + ' - 索引: ' + file.index
// 										+ ' - 文件名: ' + file.name + ' - 文件大小: '
// 										+ file.size + ' - 类型: ' + file.type
// 										+ ' - 创建日期: ' + file.creationdate
...
// 										+ errorString);
							...
							'onUploadProgress' : function(file,
									fileBytesLoaded, fileTotalBytes,
									queueBytesLoaded,
									swfuploadifyQueueUploadSize) {//上传进度发生变更时触发  
// 								alert('id: ' + file.id + ' - 索引: ' + file.index
// 										+ ' - 文件名: ' + file.name + ' - 文件大小: '
// 										+ file.size + ' - 类型: ' + file.type
// 										+ ' - 创建日期: ' + file.creationdate
...
<body>
	...
	<p>
		...
		<!-- 取消第一个未取消的文件 -->
		<a href="javascript:$('#uploadify').uploadify('cancel')">取消上传</a> <a
			href="javascript:$('#uploadify').uploadify('upload','*')">开始上传所有文件</a>&nbsp;
